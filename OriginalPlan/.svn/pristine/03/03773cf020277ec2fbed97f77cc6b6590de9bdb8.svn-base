...
int ${signature_name}(lua_State* tolua_S)
{
    ...
    if (!tolua_isusertable(tolua_S,1,"${generator.scriptname_from_native($namespaced_class_name, $namespace_name)}",0,&tolua_err)){
		goto tolua_lerror;
	}
    ...
    #end while
#end if
    gxError("${generator.scriptname_from_native($namespaced_class_name, $namespace_name)}:${func_name} has wrong number of arguments: {0}, was expecting {1}\n ", argc, ${min_args});
    return 0;
