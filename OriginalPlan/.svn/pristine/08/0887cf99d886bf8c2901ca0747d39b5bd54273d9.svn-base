...
return {
  ...
  api = {"baselib","mapserver","worldserver","corelib","EnumDefines","lua_cocos2dx_auto_e"},
  ...
  takeparameters = true,
	getEntryPoint = function(self,workdir)
		local fworkddir = workdir
	  local entryfile = nil
    local epoints = ide.config.lua and ide.config.lua.entrypoints
    if epoints then
      epoints = type(epoints) == 'table' and epoints or {epoints}
      for _,entry in pairs(epoints) do
        entryfile = GetFullPathIfExists(fworkddir, entry)
        if entryfile then break end
      end
