...
int ${signature_name}(lua_State* tolua_S)
{
    ...
    if (!cobj)
    {
        tolua_error(tolua_S,"invalid 'cobject' in function '${signature_name}'", nullptr);
        return 0;
    }
#end if
...
#end if
#end for
    gxError("${generator.scriptname_from_native($namespaced_class_name, $namespace_name)}:${func.func_name} has wrong number of arguments: {0}, was expecting {1} \n", argc, ${func.min_args});
    return 0;
