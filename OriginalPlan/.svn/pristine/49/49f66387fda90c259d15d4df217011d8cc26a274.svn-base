...
local rand_nums = {}
	...
	for i=1, nums do
		...
		item = math.random(100000000) + math.random(100000000)

		table.insert(rand_nums, item)
...
for k,v in ipairs(rand_nums) do

		if heap1.size < 10 then

			heap1:insert(v, v);

	elseif heap1:get_min() < v then
...
for k=nums-10+1,nums do

		heap1:delete_min();

		assert(table1[k] ~= heap1:get_min());
