...
-- auto scale mode
CONFIG_SCREEN_AUTOSCALE = "FIXED_WIDTH"-- design resolution
...
-- if h/w > CONFIG_SCREEN_HEIGHT/CONFIG_SCREEN_WIDTH then
...
elseif CONFIG_SCREEN_AUTOSCALE then
    ...
    elseif CONFIG_SCREEN_AUTOSCALE == "FIXED_HEIGHT" then
        if h/w > CONFIG_SCREEN_HEIGHT/CONFIG_SCREEN_WIDTH then
            local s = CONFIG_SCREEN_HEIGHT/CONFIG_SCREEN_WIDTH
            CONFIG_SCREEN_WIDTH = w / wscale
            CONFIG_SCREEN_HEIGHT = CONFIG_SCREEN_WIDTH*s
            -- CONFIG_SCREEN_WIDTH = w
        else
            CONFIG_SCREEN_WIDTH = w / scale
        end
    ...
    -- CONFIG_SCREEN_HEIGHT = h
    -- CONFIG_SCREEN_WIDTH = CONFIG_SCREEN_WIDTH*w/CONFIG_SCREEN_HEIGHT
...
function display.newScene(name)
    local scene = CCSceneExtend.extend(RendScene:create())
...
function display.newLayer()
    local layer =  CCLayerExtend.extend(CCLayer:create())
...
function display.newNode()
    local node = CCNodeExtend.extend(CCNode:create())
    -- if node.aaaa then
    --     assert(not  node.aaaa,"ri..............node.aaaa........."..node.aaaa)
    -- else
    --     node.aaaa = 222222222222
    -- end
    return node
end
...
require("framework.cocos2dx.XSpriteExtend")
function display.newXSprite( filename,x,y )
    local sprite
    if not filename then
        sprite = XSprite:create()
    elseif type(filename) == "string" then
        if string.byte(filename) == 35 then -- first char is #
            sprite = XSprite:createWithImage(string.sub(filename, 2))
        else
            filename = display.changeFileName(filename)
            sprite = XSprite:createWithPath(filename)
        end
    end
    -- if sprite.aaaa then
    --     assert(not sprite.aaaa,"aa:"..sprite.aaaa.."ri..............xsprite.aaaa.........")
    -- else
    --    sprite.aaaa = 11111111
    -- end
    if sprite then
        XSpriteExtend.extend(sprite)
        if x and y then sprite:setPosition(x, y) end
    end
    return sprite
end

function display.newLightSprite(filename)
    local sprite = CLightSprite:create()
...
require("framework.cocos2dx.XClipSpriteExtend")
function display.newXClipSprite()
    local sprite = XClipSprite:create()
    XClipSpriteExtend.extend(sprite)
    return sprite
end

require("framework.cocos2dx.XTileExtend")
function display.newXTileSprite(imageUrl,size)
    local tile = CCNode:create()
    XTileExtend.extend(tile)
    if imageUrl then
        tile:setSpriteImage(imageUrl,size)
    elseif size then
...
function display.changeFileName(name)
    if device.isLowApp then
        return string.gsub(name,"%.pvr.ccz","%.mpng")
    end
    return name
end
