...
function M.get_keywords(ast, src)
  ...
  local oast =
    (ast.tag == 'Op' and #ast == 3 and tostring(ast[2].lineinfo.first):match('|K(%d+)') > tostring(ast[3].lineinfo.first):match('|K(%d+)'))
  ...
  while i <= #ast do
    ...
    if i == 0 then  -- before first child
      fpos = tonumber(tostring(ast.lineinfo.first):match('|K(%d+)'))
    else
      local last = oast[i].lineinfo.last; local c = last.comments
      fpos = (c and #c > 0 and c[#c][3] or tostring(last):match('|K(%d+)')) + 1
    end
    local lpos
    if j == #ast+1 then  -- after last child
      lpos = tonumber(tostring(ast.lineinfo.last):match('|K(%d+)'))
    else
      local first = oast[j].lineinfo.first; local c = first.comments
      lpos = (c and #c > 0 and c[1][2] or tostring(first):match('|K(%d+)')) - 1
    end
...
local isterminal = {Nil=true, Dots=true, True=true, False=true, Number=true, String=true,
  Dots=true, Id=true}
local function compare_tokens_(atoken, btoken)
--	assert(atoken.fpos ~= nil, atoken[1], atoken[2], atoken[3]);
--	assert(btoken.fpos ~= nil, btoken[1], btoken[2], btoken[3]);
	atoken.fpos = tonumber(atoken.fpos);
	btoken.fpos = tonumber(btoken.fpos);
	--atoken.fpos = atoken.fpos ~=nil and atoken.fpos or 0;
	--btoken.fpos = btoken.fpos ~=nil and btoken.fpos or 0;
	return atoken.fpos < btoken.fpos
end
  ...
  M.walk(top_ast, function(ast)
    if isterminal[ast.tag] then -- Extract terminal
      ...
      if ast.lineinfo then
        token.fpos = tonumber(tostring(ast.lineinfo.first):match('|K(%d+)'))
        token.lpos = tonumber(tostring(ast.lineinfo.last):match('|K(%d+)'))
        token.ast = ast
		token.fpos = tonumber(token.fpos);
		token.lpos = tonumber(token.lpos);
