local UIVList = class("UIVList", function(width, anchorx, bgImage) 
--	if bgImage then
...
function UIVList:ctor(width, anchorx, bgImage)
	self:retain()
	assert(type(anchorx) == "number")
	self._padding = {left=0, right=0, bottom=0, top=0}
	self._nodesindex = {}
	self._nodes = {}
	self._anchorx = anchorx or 0.5
	self._count = 0
	self._margin = 0
	self._width = width
...
function UIVList:setWidth(width)
	self._width = width or self._width
end

function UIVList:getContentWidth()
	return self._width - self._padding.left - self._padding.right
end

function UIVList:setAutoPos(autoPos)
	self._autoPos = tobool(autoPos)
end

function UIVList:getWidth()
	return self._width
end

function UIVList:setMargin(margin)
	self._margin = margin or self._margin
end

function UIVList:setBgImage(bgImage)
...
function UIVList:getBgImage()
	return self._bgImage
end

function UIVList:updateBgImage(siz, pos, anchor)
...
function UIVList:setAutoUpdate(auto)
	self._autoupdate = tobool(auto)
end

function UIVList:setPadding(padding)
...
--[[
function UIVList:getNowContentSize()
...
function UIVList:getNowContentSize()
	local size = CCSize(self._width, 0)
	for i, cell in ipairs(self._nodes) do
		local cellSize = cell:getContentSize()
		size["height"] = size["height"] + cellSize["height"] + self._margin
	end
	
	size.height = size.height + self._padding.bottom + self._padding.top
	return size
end

function UIVList:updateContentSize()
...
function UIVList:updatePosition()
	local x,y = 0,self._padding.bottom
	local width = self._width - (self._padding.left + self._padding.right)
	for i = #self._nodes, 1, -1 do
		local cell = self._nodes[i]
		local anchor = cell:getAnchorPoint()
		local cellSize = cell:getContentSize()
		cell:setPosition(self._padding.left+(width-cellSize.width)*(self._anchorx-anchor.x),
			y+self._padding.bottom+cellSize.height*anchor.y)
		y = y + cellSize.height + self._margin
	end
end

function UIVList:update()
	self:updateContentSize()
	--self:updatePosition()
end

function UIVList:addCell(cell)
	if not cell then return end
	if self._nodesindex[cell] then
		assert(false, "function UIVList:addCell(cell) cell already exists")
	end
	cell:retain()
	self:addChild(cell)
...
function UIVList:getCount()
	return #self._nodes
end

function UIVList:isFull()
	return self._maxnum == self:getCount()
end

function UIVList:clearCell(dispose)
	if self._nodes then
...
function UIVList:getAllCell()
	return self._nodes
end

function UIVList:clear()
	self:clearBgImage()
	self:clearCell(true)
end

function UIVList:dispose()
	self:clear()
