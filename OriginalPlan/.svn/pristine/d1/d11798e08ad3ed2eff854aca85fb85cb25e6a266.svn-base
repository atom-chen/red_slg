...
def build_namespace(cursor, namespaces=[]):
    ...
    if cursor:
        ...
        if parent:
            if parent.kind == cindex.CursorKind.NAMESPACE or parent.kind == cindex.CursorKind.CLASS_DECL or parent.kind == cindex.CursorKind.STRUCT_DECL:
...
class NativeEnum(object):
    def __init__(self, cursor, generator):
        ...
        self.class_name = self.enum_name
        self.values = []
        self.parse();
        self.parents = []
        self.comments = cursor.getRawComment();
        self.stdcomment = False
        if self.comments != None:
            tempcomments = re.compile("^\s*///\s*@brief\s+(.+)").match(self.comments)
            if tempcomments != None:
                self.comments = tempcomments.group(1)
                self.stdcomment = True
            else:
    ...
    def parse(self):
        for node in self.cursor.get_children():
            valuecomments = node.getRawComment();
            if valuecomments != None:
                tempcomments = re.compile("^\s*///<\s*(.+)").match(valuecomments)
                if tempcomments != None:
                    valuecomments = tempcomments.group(1)
            self.values.append({"name":node.displayname, "value":node.enum_value, "comments":valuecomments});

    def generate_code(self):
        config = self.generator.config
        register = Template(file=os.path.join(self.generator.target, "templates", "enum.c"),
                            searchList=[{"current_enum": self}])
        self.generator.enum_file.write(str(register))
        editor_api = Template(file=os.path.join(self.generator.target, "templates", "editor_api_enum.c"),
                            searchList=[{"current_enum": self}])
        self.generator.editor_api_file.write(str(editor_api))
        client_enum = Template(file=os.path.join(self.generator.target, "templates", "inl_client_enum.c"),
                            searchList=[{"current_enum": self}])
        self.generator.client_enum_file.write(str(client_enum))
...
class NativeClass(object):
    ...
    def generate_code(self):
        ...
        editorapi = Template(file=os.path.join(self.generator.target, "templates", "inl_editor_api.c"),
                            searchList=[{"current_class": self}])
        self.generator.editor_api_file.write(str(editorapi))
        #if self.generator.script_type == "lua":
    ...
    def _process_node(self, cursor):
        ...
        if cursor.kind == cindex.CursorKind.CXX_BASE_SPECIFIER:
            ...
            if self._current_visibility == cindex.AccessSpecifierKind.PUBLIC and not cursor.type.is_function_variadic():
                if str(cursor.spelling).find('operator') != -1 or str(cursor.spelling).find('serial') != -1 or str(cursor.spelling).find('unSerial') != -1:
                    return True
...
class Generator(object):
    def __init__(self, opts):
        ...
        self.enumtype = opts['enumtype']
        self.prefix = opts['prefix']
        self.headers = opts['headers'].split(' ')
        self.srcdir = self.replace_dir(opts['srcdir']+"/")
        self.skipfiles = opts['skipfiles'].split(' ')
        self.skipgenfiles = []
        self.handlefiles = []
        self.gensrcdir = []
        self.api_dir = opts['api_dir']
        self.classes = opts['classes']
        self.enums = opts['enums']
        ...
        self.impl_file = None
        self.head_file = None
        self.editor_api_file = None
        self.client_enum_file = None
        self.skip_classes = {}
        ...
        tempdirs = opts['gensrcdir'].split(' ')
        for key in tempdirs:
            self.gensrcdir.append(self.replace_dir(key))
        
        for key in self.skipfiles:
            for key1 in self.gensrcdir:
                if os.path.isdir(key1+key):
                    self.skipgenfiles.append(self.replace_dir(key1+key+"/"))
                elif os.path.isfile(key1+key):
                    self.skipgenfiles.append(self.replace_dir(key1+key))

    def replace_dir(self, path):
        tempstr = path
    ...
    def generate_header(self):
        fileinfo = open(self.srcdir+"/__header.h",'w')
        self.walk_dir(self.srcdir, ".", fileinfo)
        fileinfo.close()
        self.headers = [self.srcdir+"/__header.h"]

    def walk_dir(self, dir, appdir, fileinfo):
        for name in os.listdir(dir):
            if name[0] == '.':
                continue
            
            dirl = dir+"/"+name
            dirs = appdir+"/"+name if appdir != '.' else name
            
            if self.in_listed_skipfiles(dirs):
                continue

            if os.path.isdir(dirl):
                self.walk_dir(dirl, dirs, fileinfo)
            if os.path.isfile(dirl):
                sufix = os.path.splitext(name)[1][1:]
                if sufix == 'h':
                    fileinfo.write("#include \""+dirs+'\"\n')
                    self.handlefiles.append(self.replace_dir(self.srcdir+dirs))
    ...
    def in_listed_classes1(self, class_name, cursor):
        """
        returns True if the class is in the list of required classes and it's not in the skip list
        """
        if class_name == "":
        ...
        if self.should_skip(class_name, None):
            return False
        
        for key in self.skipgenfiles:
            md = re.match("^"+key, self.replace_dir(str(cursor.location.file)))
            if md:
                return False
                
        for key in self.gensrcdir:
            md = re.match("^" + key, self.replace_dir(str(cursor.location.file)))
            #if self.replace_dir(str(cursor.location.file)) == key:
            if md:
                return True

        return False

    def in_listed_enum(self, enum_name, cursor):
        """
        returns True if the class is in the list of required classes and it's not in the skip list
        """
        if enum_name == "":
        ...
        if self.should_skip(enum_name, None):
            return False
    ...
    def in_listed_skipfiles(self, filename):
        """
        returns True if the class is in the list of required classes and it's not in the skip list
        """
        for key in self.skipfiles:
            md = re.match("^" + key + "$", filename)
            if md:
                return True
        return False
    ...
    def generate_code(self):
        ...
        editorapifilepath = os.path.join(self.api_dir, self.out_file+".lua")
        self.editor_api_file = open(editorapifilepath, "w+")
        ...
        apiout_h = Template(file=os.path.join(self.target, "templates", "inl_editor_api_header.h"),
                        searchList=[self])
        self.editor_api_file.write(str(apiout_h));

        if len(self.enums) > 0:
            enumfilepath = os.path.join(self.outdir, self.out_file + ".lua")
            self.enum_file = open(enumfilepath, "w+")
            clientenumfilepath = os.path.join(self.outdir, self.out_file+"Client.lua");
            self.client_enum_file = open(clientenumfilepath, "w+");
        ...
        apiout_c = Template(file=os.path.join(self.target, "templates", "inl_editor_api_header.c"),
                        searchList=[self])
        self.editor_api_file.write(str(apiout_c));
        ...
        if len(self.classes) > 0:
            ...
            #self.doc_file.close()
            self.editor_api_file.close()
        
        if len(self.enums) > 0:
            self.enum_file.close()
            self.client_enum_file.close();
        ...
        if len(self.classes) > 0:
            if cursor.kind == cindex.CursorKind.CLASS_DECL or cursor.kind == cindex.CursorKind.STRUCT_DECL:
                if cursor == cursor.type.get_declaration() and len(cursor.get_children_array()) > 0:
                    ...
                    if is_targeted_class and self.in_listed_classes1(cursor.displayname, cursor):
                        ...
                        if not self.generated_classes.has_key(cursor.displayname):
                            ...
                            if not self.in_listed_classes(cursor.displayname):
                                self.classes.append(cursor.displayname)
                        return
        if len(self.enums) > 0:
            if cursor.kind == cindex.CursorKind.ENUM_DECL:
                if cursor == cursor.type.get_declaration() and len(cursor.get_children_array()) > 0:
                    ...
                    if is_targeted_class and self.in_listed_enum(cursor.displayname, cursor):
                        if not self.generated_classes.has_key(cursor.displayname):
                            nenum = NativeEnum(cursor, self)
                            nenum.generate_code();
                            self.generated_classes[cursor.displayname] = nenum
                        return
...
def main():
    ...
    parser.add_option("-e", action="store", type="string", dest="enumtype",
                        help="parse enum type")
    ...
    for t in targets:
        ...
        for s in sections:
            ...
            gen_opts = {
                ...
                'classes': config.get(s, 'classes').split(' ') if len(config.get(s, 'classes')) > 0 else [],
				'enums': config.get(s, 'enums').split(' ') if len(config.get(s, 'enums')) > 0 else [],
                ...
                'srcdir': config.get(s, 'srcdir', 0, dict(userconfig.items('DEFAULT'))),
                'gensrcdir':  config.get(s, 'gensrcdir', 0, dict(userconfig.items('DEFAULT'))),
				'skipfiles': config.get(s, 'skipfiles', 0, dict(userconfig.items('DEFAULT'))),
				...
				'api_dir': config.get(s, 'api_dir', 0, dict(userconfig.items('DEFAULT')))
                }
