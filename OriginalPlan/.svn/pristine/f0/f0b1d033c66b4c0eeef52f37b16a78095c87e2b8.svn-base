...
namespace GXMISC
{
	...
	bool CSString::isValidText()
	{
		...
		while (i--)
		{
			...
			{
				gxDebug("string is not valid text due to character: {0} at index: {1}",(uint32)operator[](i),i);
				return false;
			}
		}
	...
	bool CSString::splitWords(TCVectorSString& result) const
	{
		CSString s=strip();
		while(!s.empty())
		{
	...
	bool CSString::splitWordOrWords(TCVectorSString& result,bool useSlashStringEscape,bool useRepeatQuoteStringEscape) const
	{
		CSString s=*this;
		while(!s.empty())
		{
	...
	bool CSString::splitLines(TCVectorSString& result) const
	{
		CSString s=*this;
	...
	bool CSString::splitBySeparator(	char separator, TCVectorSString& result,
		bool useAngleBrace,					// treat '<' and '>' as brackets
		bool useSlashStringEscape,			// treat '\' as escape char so "\"" == '"'
		bool useRepeatQuoteStringEscape,	// treat """" as '"'
	...
	bool CSString::splitByOneOfSeparators(	const CSString& separators, TCVectorSString& result,
		bool useAngleBrace,				// treat '<' and '>' as brackets
		bool useSlashStringEscape,		// treat '\' as escape char so "\"" == '"'
		bool useRepeatQuoteStringEscape,// treat """" as '"'
	...
	CSString CSString::quote(bool useSlashStringEscape,bool useRepeatQuoteStringEscape) const
	{
		...
		for (uint32 i=0;i<size();++i)
		{
			...
			default:
				...
				{
					result+=GXMISC::gxToString("\\x%02x",(unsigned char)(*this)[i]);
					continue;
				}
				break;
			}
			...
			DBreakable
			{
				if((*this)[i]=='&')
				{
					...
					if ((size()-i)>=5)
					{
						...
						if (j>=i+6) gxWarning("Truncating extended char code {0} => using {1}", (substr(i,j-i+1), substr(j-2,2)).c_str());
	...
	bool CSString::readFromFile(const CSString& fileName)
	{
		...
		if (file==NULL)
		{
			...
			// gxwarning("Failed to open file for reading: {0}",fileName.c_str());
			return false;
		}
		resize(GXMISC::CFile::GetFileSize(file));
		uint32 bytesRead=(uint32)fread(const_cast<char*>(data()),1,size(),file);
		fclose(file);
		if (bytesRead!=size())
		{
			resize(bytesRead);
			gxWarning("Failed to read file contents (requested {0} bytes but fread returned {1}) for file: {2}", size(), bytesRead,fileName.c_str());
			return false;
		}
		return true;
	}
	...
	bool CSString::writeToFile(const CSString& fileName) const
	{
		...
		if (file==NULL)
		{
			gxWarning("Failed to open file for writing: {0}",fileName.c_str());
			return false;
		}
		...
		if (recordsWritten!=1)
		{
			gxWarning("Failed to write file contents (requested {0} bytes but fwrite returned {1}) for file: {2}", size(), recordsWritten, fileName.c_str());
			return false;
		}
		gxDebug("CSSWTF Wrote {0} bytes to file {1}",size(),fileName.c_str());
		return true;
	}
	...
	{
		// if the file exists...
		if (GXMISC::CFile::FileExists(fileName))
		{
			// the file exists so check it's the right size
			if (GXMISC::CFile::GetFileSize(fileName)==size())
			{
				...
				if (hold.size()==size() && memcmp(&hold[0],&(*this)[0],size())==0)
				{
					...
					gxDebug("CSSWTF Request to write data to file {0} IGNORED because file already contains correct data",fileName.c_str());
					return true;
				}
			}
		}
