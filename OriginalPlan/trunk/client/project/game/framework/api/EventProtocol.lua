...
function EventProtocol.extend(object)
    ...
    **Usage:**
    ...
    function object:addEventListener(eventName, listener,priority)
        if object:hasEventListener(eventName,listener) then  --已经监听了
            return
        end
	...
	function object:addEventListenerList(target, eventlist, priority)
		for eventName, handler in pairs(eventlist) do
			self:addEventListener(eventName, {target, handler}, priority)
		end
	end

	function object:removeEventListenerList(target, eventlist, priority)
		for eventName, handler in pairs(eventlist) do
			self:removeEventListener(eventName, {target, handler}, priority)
		end
	end

	function object:addEventListenerList1(eventlist, priority)
		for eventName, handler in pairs(eventlist) do
			self:addEventListener(eventName,handler, priority)
		end
	end

	function object:removeEventListenerList1(eventlist, priority)
		for eventName, handler in pairs(eventlist) do
			self:removeEventListener(eventName, handler, priority)
		end
	end
        ...
        for i = #listeners, 1, -1 do
            ...
            if listener then   --考虑这样的情况：如果在遍历时，已经被优先级更高的回调函数里面移除了
	            if type(listener) == "table" then
					if DEBUG == 2 then
						if type(listener[2]) ~= 'function' then
							dump(listener)
						end
					end
    ...
    **Syntax:**
    ...
    function object:dumpListener()
        for e,list in pairs(object.listeners) do
            print("listener",e,#list)
        end
    end
