...
table.tostring = function(t)
	...
	local function isWord(word)
		...
		if not ((b <= z and b >= a) or (b <=Z and b >= A) or b == under) then
			return false
		end

		for i=2,word:len() do
			if not ((b <= z and b >= a) or (b <=Z and b >= A) or b == under or (b >= 48 and b < 58)) then
				return false
			end
		end
...
--[=[
function string.join(str, tab)
	--print("function string.join(str, tab)", str, tab)
	local ret = ""
	for i,v in ipairs(tab) do
		if ret == "" then
			ret = tostring(v)
		else
			ret = ret .. str .. tostring(v)
		end
	end
	return ret
end
