...
return {
  ...
  marksymbols = function(editor, code, pos, vars)
    ...
    return coroutine.wrap(function()
      ...
      PARSE.parse_scope_resolve(lx, function(op, name, lineinfo, vars, nobreak)
        ...
        elseif op == 'VarNext' or op == 'VarInside' then
					-- match a=b
					local line = editor:LineFromPosition(lineinfo);
					...
					if (text:match("^%s*local%s+[%w_]+%s*=%s*[%w_]+[%s;]*$")) then
						...
						if cnext.tag == 'Id' then
							coroutine.yield(op, name, lineinfo, vars, at, cnext)
							return;
						end
					end
          ...
          table.insert(varnext, {'Var', name, lineinfo, vars, at, nobreak})
				elseif op == 'Id' then
					...
					if (text:match("^%s*[%w_]+%s*=%s*[%w_]+[%s;]*$")) then
						...
						lx:next();
						local cnext = lx:peek();
						if not cnext or cnext.tag == 'Id' then
								-- match a=b
							coroutine.yield(op, name, lineinfo, vars, at, cnext)
							return;
						end
