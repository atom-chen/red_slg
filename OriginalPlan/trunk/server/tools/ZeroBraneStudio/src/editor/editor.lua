...
function NavigateToPosition(editor, fromPosition, toPosition, length)
  --table.insert(editor.jumpstack, fromPosition)
	table.insert(AllJumpStack, {fileName=GetEditorPath(editor),lineno=fromPosition});
...
function SetEditorSelection(selection)
  ...
  if editor then
    ...
    AddToFileHistory(openDocuments[id] and openDocuments[id].filePath)
		PackageEventHandle("onEditorSelected", editor);
  else
...
function EditorAutoComplete(editor)
	...
	if editor.spec.iscomment[style] or editor.spec.isstring[style] or (MarkupIsAny and MarkupIsAny(style)) -- markup in comments
	then 
		return
	end
	...
	if nil ~= lt then
		...
		local userList = CreateAutoCompList(editor, lt, pos, true)
...
function GetValAtPosition(editor, pos, unselect)
...
function EditorCallTip(editor, pos, x, y)
  ...
  --local tip = GetTipInfo(editor, funccall or var, false, not funccall)
	local tip = GetTipInfo(editor, funccall or var, false, true, pos, true)
...
-- find all instances of a symbol at pos
-- return table with [0] as the definition position (if local)
function IndicateFindSymbolInst(editor, name, pos)
  ...
  for _, token in ipairs(tokens) do
    ...
    if op == 'EndScope' then -- EndScope has "new" level, so need +1
      ...
      if #instances > 1 and instances[#instances][-1] == token.at+1 then
        table.remove(instances)
				table.remove(symbolInst);
				assert(#instances == #symbolInst);
      end
    elseif token.name == name then
      if op == 'Id' then
        table.insert(instances[#instances], token.fpos)
				table.insert(symbolInst[#symbolInst], token);
				assert(#instances == #symbolInst);
      elseif op:find("^Var") then
        if this and this == token.at then break end
        ...
        instances[#instances + (token.at > instances[#instances][-1] and 1 or 0)]
          = {[0] = (not token.self and token.fpos or nil), [-1] = token.at}
				symbolInst[#instances + (token.at > instances[#instances][-1] and 1 or 0)]
          = {[0] = token}
				assert(#instances == #symbolInst);
      end
      if token.fpos <= pos and pos <= token.fpos+#name then this = instances[#instances][-1] end
  ...
  -- this is to avoid reporting (improper) instances when checking for
  -- comments, strings, table fields, etc.
  --this and assert(#(symbolInst[#symbolInst]) == #(instances[#instances]));
  return this and symbolInst[#symbolInst] or {}
end
...
function IndicateAll(editor, lines, linee)
  ...
  local tokens = editor:GetTokenList()
  local functions = editor:GetFunctionList() -- @My
  ...
  if start then -- if the range is specified
    ...
    if pos == 1 then -- if not continuing, then trim the list
      tokens = editor:ResetTokenList()
			functions = editor:ResetFunctionList(); -- @My
    end
  end
  ...
  local f = editor.spec.marksymbols(editor, editor:GetTextDyn(), pos, vars)
  local lastOp = nil
  while true and f do
    ...
    if op == 'Function' then
      ...
      table.insert(functions, token); -- @My
    end
    if op == 'FunctionCall' then
			--[[
      if lineinfo > 1 and editor:GetCharAt(lineinfo-2) <= string.byte(' ') then
        for kf=1,#functions,1 do
	        if functions[kf].name == name then
	          IndicateOne(indicator.GLOBAL, lineinfo, #name)
	        end
	      end
      end
			--]]
			if PkgIsGlobalCall(editor, lineinfo) then
				IndicateOne(indicator.GLOBAL, lineinfo, #name)
			end
			if GetClassNameByPos(editor, lineinfo, false) ~= nil then
				IndicateOne(indicator.LOCAL, lineinfo, #name)
			end
    ...
    if op == 'Id'
    --and (var and indic.varlocal or not var and indic.varglobal) then --@My
	  and (var and indic.varlocal) and name ~= 'self' then -- @My
...
function CreateEditor(bare)
  ...
  editor.tokenlist = {}
  editor.functions = {} -- @My
  editor:AutoCompSetMaxHeight(20); -- @My
  editor.onidle = {}
  editor.pageHistoryIndex = getHistoryIndex();
  ...
  --editor:SetSelectionMode(SC_SEL_RECTANGLE); -- @My
  ...
  function editor:ResetTokenList() self.tokenlist = {}; return self.tokenlist end
	function editor:GetFunctionList() return self.functions end --@My
	function editor:ResetFunctionList() self.functions = {}; return self.functions end --@My
  ...
  editor:Connect(wxstc.wxEVT_STC_MODIFIED,
    function (event)
      ...
      if (inserted or deleted) then
        ...
        PackageEventHandle("onEditorModified", editor);
      end
  ...
  editor:Connect(wxstc.wxEVT_STC_CHARADDED,
    function (event)
      ...
      elseif (ch == LF) then
        ...
        if (line > 0) then
          ...
          editor:GotoPos(editor:GetCurrentPos()+indent)
			...
			elseif ch == ("("):byte() then
				--local linetx = linetxtopos:match("=?%s*(.+)%(");
				local linetx = GetValAllAtPosition(editor, editor:GetCurrentPos(), true);
				if linetx then
					local tip = GetTipInfo(editor,linetx,ide.config.acandtip.shorttip,true,pos-1,true)
					if tip then
						if editor:CallTipActive() then editor:CallTipCancel() end
						if not PackageEventHandle("onEditorCallTip", editor, tip) then
      ...
      elseif ide.config.autocomplete then -- code completion prompt
        ...
        editor.autocomplete = trigger and (#trigger > 0 or trigger:match("["..editor.spec.sep.."]"))
          and true or false
          ...
          editor:DoWhenIdle(function(editor)
            if PackageEventHandle("onEditorAutoComplete", editor) == false then
            else
  ...
  --editor:Connect(wx.wxEVT_LEFT_UP,
  --  function (event)
  --    local position = editor:PositionFromPointClose(event:GetX(),event:GetY())
  --
  --    if position ~= wxstc.wxSTC_INVALID_POSITION then
  --      PackageEventHandle("onLeftUp", editor, position);
  --    end
  --
  --    event:Skip();
  --  end
  ...
  editor:Connect(wx.wxEVT_LEFT_UP,
    function (event)
      ...
      --if event:ControlDown() and not event:AltDown() -- @My
	...
	if not PkgToDefine(editor, editor:GetCurrentPos()) then
      ...
      --if event:AltDown() and not event:ControlDown() -- @My
      -- ide.wxver >= "2.9.5"; fix after GetModifiers is added to wxMouseEvent in wxlua
      --and not event:ShiftDown() and not event:MetaDown() then
      --  PkgNavigateBack()
      --  event:Skip()
  ...
  editor:Connect(wx.wxEVT_KEY_UP,
    function (event)
	  if event:GetKeyCode() ~= wx.WXK_TAB and event:GetKeyCode() ~= wx.WXK_PAGEUP and event:GetKeyCode() ~= wx.WXK_PAGEDOWN then
    ...
    function (event)
      ...
      elseif mod == wx.wxMOD_ALT and keycode == wx.WXK_LEFT then
        ...
        --if not navigateBack(editor) then event:Skip() end
				if not PkgNavigateBack() then event:Skip() end
    ...
    function(event)
      ...
      -- only activate selection of instances on Ctrl/Cmd-DoubleClick
			...
			if instances and (instances[0] or #instances > 0) then
				...
				PkgSelectKey(editor, pos, event:GetModifiers() == wx.wxMOD_CONTROL);
			end
			if event:GetModifiers() == wx.wxMOD_CONTROL then
				PackageEventHandle("onEditorDClicked", editor, false);
			else
				PackageEventHandle("onEditorDClicked", editor, true);
			end
	  --]]
      event:Skip()
    end)
