...
set SERVER_PATH=E:/backupcode/gd/server/
...
python %CXX_GENERATOR_ROOT%/generator.py %CXX_GENERATOR_ROOT%/build/mapserver.ini -t lua -s mapserver -i 1
python %CXX_GENERATOR_ROOT%/generator.py %CXX_GENERATOR_ROOT%/build/worldserver.ini -t lua -s worldserver -i 1
python %CXX_GENERATOR_ROOT%/generator.py %CXX_GENERATOR_ROOT%/build/enums.ini -t lua -s enumexp -i 1
...
echo ---------------------------------
copy %SERVER_PATH%\trunk\exe\mapserver\script\code\server\EnumDefines.lua %SERVER_PATH%\trunk\exe\coredll\EnumDefines.lua
endlocal
exit /b 0
