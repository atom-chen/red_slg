...
int ${signature_name}(lua_State* tolua_S)
{
    ...
    cobj = lua_tinker::read<${namespaced_class_name}*>(tolua_S,1);
    if (!cobj)
    {
        tolua_error(tolua_S,"invalid 'object' in function '${signature_name}'", nullptr);
        return 0;
    }
#end if
