...
function ${current_class.class_name}:Read(stream)
	...
	#else
	${sp}local _${m.name}_${typenum}_o = {}
	${sp}${m.itertype[$typenum-1]['typestr']}.Read(_${m.name}_${typenum}_o, stream)
	...
	#else
	${sp}self.${m.name} = {}		-------- $m.comments
	${sp}${m.itertype[$typenum-1]['typestr']}.Read(self.${m.name}, stream)
	#end if
	#end if 
	#set count = $typenum-1
	#while $count > 0
...
function ${current_class.class_name}:Write(stream)
	...
	#else
	${sp}${m.itertype[$typenum-1]['typestr']}.Write(_${m.name}_${count}_t, stream)
	#end if
	#else
	#if $memtype == "base"
	...
	${sp}${m.itertype[$typenum-1]['typestr']}.Write(self.${m.name}, stream)
	#end if
	#end if
	#set count = $typenum-1
	#while $count > 0
