...
function httpGenHeader()
	...
	--header = header .. "<head><meta http-equiv=\"Content-Type\" content=\"text/html;charset=utf-8\"/></head>";
...
function httpHandleFunction(options, body, params, funcName)
	...
	local retString, needGenHeader = func(params);
	if not needGenHeader then
		return retString;
	end
