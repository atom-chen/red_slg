...
namespace	GXMISC
{
	// @TODO 重新整理并测试可用性, 如果有必要就去掉
	// advanced class declaration...
	//class CVectorSString;
	class CSString;
	typedef std::vector<CSString> TCVectorSString;
	...
	*/
	...
	public:
		...
		///	ctor
		CSString(const std::vector<GXMISC::CSString>& v,const std::string& separator="\n");
		...
		bool splitWords(TCVectorSString& result) const;

		/// Append the individual "wordOrWords" elements in the string to the result vector
		/// retuns true on success
		bool splitWordOrWords(TCVectorSString& result,bool useSlashStringEscape=true,bool useRepeatQuoteStringEscape=true) const;

		/// Append the individual lines in the string to the result vector
		/// retuns true on success
		bool splitLines(TCVectorSString& result) const;

		/// Append the separator-separated elements in the string to the result vector
		/// retuns true on success
		bool splitBySeparator(	char separator, TCVectorSString& result,
			bool useAngleBrace=false,				// treat '<' and '>' as brackets
			bool useSlashStringEscape=true,			// treat '\' as escape char so "\"" == '"'
			bool useRepeatQuoteStringEscape=true,	// treat """" as '"'
		...
		/// Append the separator-separated elements in the string to the result vector
		/// retuns true on success
		bool splitByOneOfSeparators(	const CSString& separators, TCVectorSString& result,
			bool useAngleBrace=false,				// treat '<' and '>' as brackets
			bool useSlashStringEscape=true,			// treat '\' as escape char so "\"" == '"'
			bool useRepeatQuoteStringEscape=true,	// treat """" as '"'
		...
		{
			operator +=(GXMISC::gxToString(value));
	...
	*/
	...
	inline CSString::CSString(const std::vector<GXMISC::CSString>& v,const std::string& separator)
	{
	...
	{
		return gxStricmp(c_str(),other.c_str())==0;
	}
	...
	{
		return gxStricmp(c_str(),other)==0;
	}
...
struct CUnsensitiveSStringLessPred : public std::less<GXMISC::CSString>
{
	bool operator()(const GXMISC::CSString& x, const GXMISC::CSString& y) const { return x < y; /*.icompare(y);*/ }
};
