...
public:
	...
	uint32   getLength(TSex_t sex)
	{
		if (sex == SEX_TYPE_MALE)
		{
			return (uint32)randManRoleAry.size();
		}
		return (uint32)randWomanRoleAry.size();
	}
public:
	uint8     id;
...
class  CRandRoleNameTblLoader : public CConfigLoader<CRandRoleNameTblLoader, CRoleNameRandTbl>
{
public:
...
#define   DRandRoleNameLoader   CRandRoleNameTblLoader::GetInstance()
